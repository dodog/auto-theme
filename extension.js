...
export default class AutoThemeExtension extends Extension {
    enable() {
        ...
        this._settingsChangedId = this._settings.connect('changed', () => {
            // A manual time edit changes when the next boundary is, so
            // re-check now and re-arm the timer for the new target time.
    ...
    _applyTheme(mode) {
        ...
        this._trySetUserTheme(shellTheme);

        // --- libadwaita (GTK4) relink + custom CSS ---
        if (this._settings.get_boolean('apply-libadwaita-fix'))
            this._relinkLibadwaita(gtkTheme);

        // --- qt5ct / qt6ct style ---
        if (this._settings.get_boolean('apply-qt-fix'))
            this._applyQtStyle(mode);

        // --- restart apps that cache the old theme ---
        if (this._settings.get_boolean('restart-apps'))
            this._restartApps();

        const qtNote = this._settings.get_boolean('apply-qt-fix')
            ? `, qt=${isDark ? this._settings.get_string('qt-dark-style') : this._settings.get_string('qt-light-style')}`
            : '';
    ...
    _relinkLibadwaita(gtkTheme) {
        ...
        // Re-point the symlinks fresh at the chosen variant.
        this._symlink(GLib.build_filenamev([themeDir, 'gtk.css']), GLib.build_filenamev([cfgDir, 'gtk.css']));
        this._symlink(GLib.build_filenamev([themeDir, 'gtk-dark.css']), GLib.build_filenamev([cfgDir, 'gtk-dark.css']));
        ...
        if (this._settings.get_boolean('apply-custom-css')) {
            ...
            this._appendCssOnce(GLib.build_filenamev([cfgDir, 'gtk.css']), css);
            this._appendCssOnce(GLib.build_filenamev([cfgDir, 'gtk-dark.css']), css);
        }
    }
    ...
    _isDanglingSymlink(path) {
        ...
        return info && info.get_is_symlink();
    }
